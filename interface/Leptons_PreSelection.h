...
vector<myobject> GoodMuon(myevent *m) {
    ...
    for (int i = 0; i < muon.size(); i++) {
        ...
        if (muPt > 10 && TMath::Abs(muEta) < 2.4 && Id_Mu_Tight(muon[i]) && Iso_Mu_dBeta(muon[i]) < 0.15)
...
vector<myobject> GoodElectron(myevent *m) {
    ...
    for (int i = 0; i < electron.size(); i++) {
        ...
        if (elePt > 10 && TMath::Abs(eleEta) < 2.5 && EleMVANonTrigId_Tight(electron[i]) && Iso_Ele_dBeta(electron[i]) < 0.3)
...
int bjet_Multiplicity(myevent *m) {
    ...
    for (int k = 0; k < jet.size(); k++) {
        ...
        if (jet[k].pt > 20 && jet[k].jetId_loose && TMath::Abs(jet[k].eta) < 2.4 && jet[k].bDiscriminatiors_CSV > 0.898) {
            bool Nooverlap = true;
...
vector <myobject> myCleanBareLepton(myevent *m, string lep) {
    ...
    if (lep == "mu") {
        for (int a = 0; a < Muon_Vector.size(); a++) {
            ...
            for (int b = 0; b < goodmu_.size(); b++) {
                if (deltaR(Muon_Vector[a], goodmu_[b]) < 0.4 && deltaR(Muon_Vector[a], goodmu_[b]) > 0.05)
                    Keep_obj = false;
            }
            if (Keep_obj == true)
                mu_.push_back(Muon_Vector[a]);
    ...
    if (lep == "ele") {
        for (int a = 0; a < Electron_Vector.size(); a++) {
            bool Keep_obj = true;
            for (int b = 0; b < goodmu_.size(); b++) {
                if (deltaR(Electron_Vector[a], goodmu_[b]) < 0.4)
                    Keep_obj = false;
            }
            if (Keep_obj == true)
                electron_.push_back(Electron_Vector[a]);
    ...
    if (lep == "tau") {
        for (int a = 0; a < Tau_Vector.size(); a++) {
            ...
            for (int b = 0; b < goodelectron_.size(); b++) {
                if (deltaR(Tau_Vector[a], goodelectron_[b]) < 0.4)
                    Keep_obj = false;
            }
            for (int c = 0; c < goodmu_.size(); c++) {
                if (deltaR(Tau_Vector[a], goodmu_[c]) < 0.4)
                    Keep_obj = false;
            }
            if (Keep_obj == true)
                tau_.push_back(Tau_Vector[a]);
